...
Citizen.CreateThread(function()
    ...
    AddTextComponentString("[~p~Thor~w~] Job Miner")
	...
	while true do
		...
		if(Vdist(pos.x, pos.y, pos.z, jobPoint[1], jobPoint[2], jobPoint[3]) < 15.0)then
			...
			if hasJob == false then
				if(Vdist(pos.x, pos.y, pos.z, jobPoint[1], jobPoint[2], jobPoint[3]) < 3.0) and not finisJob then
					notifica("[~o~Thor~w~] Apasa ~INPUT_CONTEXT~ pentru a lua jobul de ~p~Miner")
					if IsControlJustPressed(1,51) then
						hasJob = true
						txt("[~o~Thor~w~] ~g~Felicitari! ~w~Acum esti ~y~Miner~w~!")
		...
		if hasJob == true and incepeSaLucrezi == true then
			if(Vdist(pos.x, pos.y, pos.z, -631.22332763672,5050.9799804688,144.10287475586) < 55.0)then
				for i,v in pairs(pietre) do
					...
					if(Vdist(pos.x, pos.y, pos.z, v[1], v[2], v[3]) < 3.0)then
						DrawText3d(v[1], v[2], v[3]+0.4, "Apasa ~r~E ~w~pentru sparge pietre", 255)
						if IsControlJustPressed(1,51) then
							animPickAxe()
							-- Wait(20000)
							txt("Du pietrele la prelucrat")
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, processLoc[1], processLoc[2], processLoc[3]) < 6.5) and pietreTajate then
				notifica("Apasa ~INPUT_CONTEXT~ pentru a cobori din masina!")
				if IsControlJustPressed(1,51) and pietreTajate then
					...
					txt("Descarca toate pietrele din masina pentru a fii prelucrati!")
				end
				txt("[~o~Thor~w~]Asteapta ca lucratorii sa-si termine munca!")
				Wait(10000)
				txt("[~o~Thor~w~]Pentru ca ai asteptat, muncitori s-au decis sa-ti incarce masina!")
				prelucrat = true
				SetNewWaypoint(jobPoint[1], jobPoint[2])
				txt("[~o~Thor~w~] Du toate pietrele prelucrate inapoi la sediu pentru a lua banii!")
			...
			if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, baza[1], baza[2], baza[3]) < 20.0) and prelucrat then
				...
				notifica("Apasa ~INPUT_CONTEXT~ pentru a vinde pietrele!")
				if IsControlJustPressed(1,51) then
			...
			if(Vdist(pos.x, pos.y, pos.z, carPos.x , carPos.y, carPos.z) > 105.0) and hasJob and incepeSaLucrezi and not DoesEntityExist(veh) then
				...
				finisJob = false
				pietreTajate = false
				txt("[~o~Thor~w~]Te-ai indepartat de masina prea mult si ai pierdut misiunea!")
			end
		end
	end
end)
...
function CreateMinerCar(x,y,z,heading) -- van
	local hash = GetHashKey("TipTruck2")
    local n = 0
